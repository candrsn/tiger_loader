...
CREATE INDEX polypt_poly_ind on tgr_work.polypt(polyid,cenid);
CREATE INDEX polyattr_poly_ind on tgr_work.polyattr(polyid,cenid);
...
SELECT st_astext(geom),cenid, polyid 
  FROM tgr_work.gpoly where st_numgeometries(geom) = 0;
...
SELECT c.tlid, count(*) as members
 FROM tgr_work.chain c,
    tgr_work.polylink p
 WHERE c.tlid = p.tlid
 GROUP BY
...
CREATE INDEX poly_polyid__ind on tgr_work.poly(cenid,polyid);
SELECT cenid, polyid 
  FROM tgr_work.polyattr a
  WHERE not exists (select 1 from tgr_work.poly p 
           where p.cenid = a.cenid and p.polyid = a.polyid);

CREATE INDEX poly_polyid__ind on tgr_work.gpoly(cenid,polyid);
SELECT cenid, polyid 
  FROM tgr_work.polyattr a
  WHERE not exists (select 1 from tgr_work.gpoly p 
           where p.cenid = a.cenid and p.polyid = a.polyid);

SELECT cenid, polyid 
  FROM tgr_work.gpoly p
  WHERE not exists (select 1 from tgr_work.polyattr a 
           where p.cenid = a.cenid and p.polyid = a.polyid);
