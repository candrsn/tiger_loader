...
# import_tgr1992.sh
#   A loader wrapper script to import TIGER 1992 data
...
#        echo "skipping $ctyfp as it is already loaded"
    ...
    # test for bad polygons
      echo "
        ...
        CREATE INDEX poly_${stfp}__geom__ind on t1992.poly_$stfp using GIST(geom);
        CREATE INDEX polypt_${stfp}__polyid__ind on t1992.polypt_$stfp (polyid,cenid);
        CREATE INDEX poly_${stfp}__polyid__ind on t1992.poly_$stfp (polyid,cenid);
